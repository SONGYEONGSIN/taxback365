...
| 2026-05-07T05:30:00Z | A-T1 | done | `eslint --fix`로 prefer-const 4건 + 미사용 disable directive 자동 수정 |
| 2026-05-07T05:35:00Z | A-T2 | done | calculator 변수 7건 분석 — 모두 dead code 확정 (UI 종합값 미표시 의도). 비즈니스 누락 의심분(transport/market/culture excess, totalDeduction) 모두 환급 계산 별도 함수에서 처리되므로 안전 삭제 |
| 2026-05-07T05:45:00Z | A-T3 | done | 잔여 unused-vars 일괄 처리. cascade로 추가 발견된 *Deduction 7건(donations IIFE) + exceeds + AIAlert + salary 매개변수 모두 정리. lib 파일 dead code(미구현 흔적: totalDonation, taxCreditRate, socialInsurance, totalCredit) 삭제 |
| 2026-05-07T05:55:00Z | A-T4 | partial | `<img>` 2건은 base64/외부 동적 URL 호환성 우려로 `eslint-disable-next-line` 주석 + follow-up 코멘트로 처리. 본격 next/image 전환은 별도 issue |
| 2026-05-07T06:00:00Z | A-T5 | done | A-T1과 함께 자동 처리됨 |
| 2026-05-07T06:02:00Z | A-T7 | done | `supabase/schema.sql` line ~61 `'FAQ'` → `'Q&A'` |
| 2026-05-07T06:05:00Z | A-T8 | done | `npm i fast-xml-parser` + `app/api/news/route.ts` 정규식 → `XMLParser`. CDATA 자동 처리 |
| 2026-05-07T06:08:00Z | — | revise | `generateRecommendationsFromAnalysis` 시그니처 변경(salary 매개변수 제거) 영향으로 `app/dashboard/page.tsx:292` 호출지점도 수정 |
| 2026-05-07T06:10:00Z | A-T6 | done | `next.config.mjs`의 `eslint.ignoreDuringBuilds` 옵션 제거. `npm run build` 통과 (lint 0 errors + tsc 0 errors) |
| FOLLOW-UP | — | open | (1) `<img>` → `next/image` 본격 전환(Navigation avatar/admin OCR 미리보기, lh3.googleusercontent.com 도메인 추가 + Google avatar 이미지 화이트리스트). (2) `lib/ai-recommendation.ts:430` taxCreditRate 미구현 비즈니스 로직(추가 납입 가능 금액 역산)이 dead code로 제거됨 — 실제 구현 별도 issue. (3) `lib/tax-store.ts:651` 의료비 totalCredit 종합값 UI 표시 미구현. (4) `app/calculator/page.tsx`의 *Deduction 7개 IIFE 변수도 종합값 표시 미구현 흔적 — UI에 종합 환급액 표시 추가 시 재구현 |
| 2026-05-07T06:12:00Z | A-T9 | deferred | CSP 운영 모니터링은 30분 트래픽 세션 필요 — 운영 사용 중 자연 검증 |
