...
| 2026-05-06T15:00:00Z | C | T12 | done | `npm i @upstash/ratelimit @upstash/redis zod` |
| 2026-05-06T15:05:00Z | C | T13 | done | `lib/rate-limit.ts` — Upstash env 부재 시 limiter null 반환 fallback |
| 2026-05-06T15:10:00Z | C | T14 | done | `lib/api-guard.ts` — `withApiGuard()` HOF (auth 401 → ratelimit 429 → zod 400 순) |
| 2026-05-06T15:30:00Z | C | T15-T17 | done | 6 라우트(ai-advice POST 10/min, ocr POST 5/min, biz-check POST 30/min, hospital/market/pharmacy POST+PUT 30/min) withApiGuard 적용 + zod schema. dev 서버 재시작(포트 3002) 후 6 라우트 모두 인증 없이 401 응답 검증 통과 |
| 2026-05-06T15:35:00Z | C | T18 | done | `.env.example` 신규 작성 (NextAuth/Supabase/Upstash/외부 API 전체 변수 문서화) |
| 2026-05-06T16:00:00Z | D | T19 | done | `npm i react-markdown remark-gfm rehype-sanitize` |
| 2026-05-06T16:05:00Z | D | T20 | done | `app/dashboard/page.tsx:1015~1026` `dangerouslySetInnerHTML` → `<ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]}>` + h1~h4/strong/p/ul/ol/li/code 컴포넌트 매핑 (기존 시각 동등) |
| 2026-05-06T16:10:00Z | D | T21 | done | `components/seo/JsonLd.tsx` 두 곳의 `JSON.stringify(jsonLd)` → `.replace(/</g, "\\u003c")` 적용 (script-breakout 차단) |
| 2026-05-06T16:15:00Z | D | T22 | deferred | 사용자 요청에 따라 AI 조언 모달 시각 회귀는 Phase E 완료 후 T33 통합 회귀에서 일괄 검증 (Gemini API 호출 비용·중복 작업 회피) |
| 2026-05-07T09:00:00Z | E | T23 | done | `next.config.mjs` `headers()` 추가 — CSP(supabase/gemini/google/upstash/data.go.kr 화이트리스트), HSTS(2y preload), X-Frame-Options DENY, X-Content-Type-Options nosniff, Referrer-Policy, Permissions-Policy |
| 2026-05-07T09:05:00Z | E | T24 | done | `compiler.removeConsole`(production, exclude error) 추가 — PII 누수 방지 |
| 2026-05-07T09:10:00Z | E | T25 | done | `app/opengraph-image.tsx`의 `runtime = "edge"` 제거. 빌드에서 `○ /opengraph-image` Static으로 표시(이전 ƒ Edge), 빌드 경고 사라짐 |
| 2026-05-07T09:15:00Z | E | T26 | done | `app/api/tax-data/route.ts` POST에 zod (`{ data: z.record }`) |
| 2026-05-07T09:20:00Z | E | T27 | done | `app/api/admin-data/route.ts` GET year query coerce + 범위 검증, POST `{ year: 2020~2030, data: record }` zod |
| 2026-05-07T09:25:00Z | E | T28 | done | `app/api/board/route.ts` POST zod (`title 1~200, content 1~50000, category, is_public`) |
| 2026-05-07T09:30:00Z | E | T29 | done | `app/api/board/route.ts` GET `or()` 보간에 SAFE_EMAIL_RE(`,()<>` 차단) 가드. 정상 이메일은 RFC상 영향 없고, 비정상 형식이면 공개 글만 반환하여 인젝션 차단 |
| 2026-05-07T09:35:00Z | E | T30 | partial | `npm audit fix` 실행. next 직접 high(DoS) vuln 해소. postcss(next 내부 transitive)는 next 다음 minor 업그레이드까지 보존, xlsx는 fix 없음 → T32 follow-up |
| 2026-05-07T09:40:00Z | E | T31 | deferred | news 라우트 fast-xml-parser 도입은 LOW + 새 의존성. 별도 chore plan으로 분리 (follow-up 섹션) |
| 2026-05-07T09:45:00Z | E | T33 | deferred | AI 조언 모달 + 회귀 시나리오 5종은 사용자 e2e 검증 (별도 라운드) |
| 2026-05-07T09:50:00Z | E | T34 | revise | 사용자 선택(β)에 따라 DoD 완화 — Phase E는 보안 영향 항목 완료를 마감 기준으로 정의. 빌드 lint `ignoreDuringBuilds: true` 유지 + 26 잔여 ESLint errors는 별도 chore plan(아래 follow-up 섹션)으로 분리. `npm run build` + `npx tsc --noEmit` 0 errors는 달성 |

## Follow-up (별도 plan 권장)

본 보안 plan 범위 외로 분리된 후속 작업:

1. **ESLint 잔여 26 errors 정리** — `prefer-const` 4건은 `--fix`, `@typescript-eslint/no-unused-vars` 22건은 비즈니스 의미 검증 필요(특히 `app/calculator/page.tsx`의 환급 계산 누락 변수 의심), `@next/next/no-img-element` 2건 → `next/image` 전환, 미사용 eslint-disable 1건. 정리 후 `next.config.mjs`의 `eslint.ignoreDuringBuilds` 제거.
2. **xlsx 패키지 교체** — `node_modules/xlsx`의 prototype pollution + ReDoS는 자동 fix 없음. `exceljs` 등 대체 패키지로 이전. 영향 범위는 OCR/엑셀 업로드 흐름.
3. **schema.sql `category` CHECK 동기화** — `add_is_public_and_rename_faq.sql`이 `'FAQ' → 'Q&A'` 변경했으나 `supabase/schema.sql`의 CHECK 제약은 'FAQ' 잔존.
4. **News 라우트 fast-xml-parser** — RSS 정규식 파싱 → 표준 XML 파서.
5. **CSP enforce-only 정책 검증** — 운영 배포 후 브라우저 콘솔에 CSP 위반 보고가 없는지 확인. 실제 위반 시 `connect-src`/`script-src` 화이트리스트 확장.
6. **감사 로그 (audit log) 테이블** — 보안 보고서 A09 MEDIUM. 로그인/관리자 작업/게시판 삭제 등 추적.
