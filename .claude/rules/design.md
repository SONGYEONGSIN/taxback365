...
> 본 프로젝트는 Tailwind v4 + `@theme inline` 방식이며 토큰의 1차 출처는 `app/globals.css`다. `lib/design-tokens.ts`는 동일 값을 TS `as const`로 미러링하여 컴포넌트 import 사용 시 타입 안전성을 제공한다.

## 디자인 토큰

- 프로젝트 디자인 토큰의 1차 출처는 `app/globals.css`의 `:root` CSS 변수 + `@theme inline` 블록 (Tailwind v4 config-less)
- 동일 토큰을 `lib/design-tokens.ts`에 `as const` TS 객체로 미러링 (컴포넌트 import용)
- 새 색상/간격/폰트 추가 시 `globals.css`와 `design-tokens.ts` 양쪽에 동기화
- 별도 `tailwind.config.ts`는 사용하지 않음 (Tailwind v4 `@theme inline` 사용)
...
- 예외: `lib/design-tokens.ts`, `app/globals.css`의 CSS 변수/`@theme inline` 정의부

## 공통 컴포넌트

- 동일 UI 패턴이 3회 이상 반복되면 `components/common/`으로 추출
- 공통 컴포넌트 후보: 검색/필터 바, 테이블 헤더, 상태 뱃지, 확인 모달, 페이지네이션
- 추출 시 Props 인터페이스 정의, barrel export(`index.ts`) 포함
- `components/ui/`는 원자(atom) 수준 — shadcn 기본 컴포넌트
- `components/common/`은 조합(molecule) 수준 — 프로젝트 공통 패턴
